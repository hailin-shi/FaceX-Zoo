...
def show_curves_multi_file(log_files, loss_names):
    ...
    unit_size = 3
    figure = plt.figure(figsize=[unit_size*(num_loss+2), unit_size * 3])
    ...
    for loss_id in range(num_loss):
        ...
        # compute media curve if needed
        eval_num_total = 0
        for k, v in curves_dict.iteritems():
            if 'x_train' in v:
                x_train, y_train = v['x_train'], v['y_train']
                eval_num_total += len(y_train)
        window_size = eval_num_total // 20 + 1
        for k, v in curves_dict.iteritems():
            if 'x_train' in v:
                x_train, y_train = v['x_train'], v['y_train']
                y_train_avg = []
                eval_num = len(y_train)
                for id_y in range(eval_num):
                    id_begin = max(0, id_y - window_size)
                    id_end = min(eval_num-1, id_y + window_size)
                    target_range = y_train[id_begin : id_end]
                    target_range = np.array(target_range)
                    target_y = target_range.mean()
                    y_train_avg.append(target_y)
                v['y_train_avg'] = y_train_avg

        figure.add_subplot(3, num_loss, loss_id + 1)
        ...
        figure.add_subplot(3, num_loss, loss_id + 1 + num_loss)
        plt.ylim([min_y, max_y])
        plt.xlabel('iterations')
        plt.ylabel(loss_name)
        i = 0
        ...
        figure.add_subplot(3, num_loss, loss_id + 1 + num_loss * 2)
        plt.ylim([min_y, max_y])
        plt.xlabel('iterations')
        plt.ylabel(loss_name)
        i = 0
        for k, v in curves_dict.iteritems():
            if 'y_train_avg' in v:
                plt.plot(v['x_train'], v['y_train_avg'], color[i], linewidth=2)
            i += 1
        plt.title(''.join([loss_name, ' vs ', 'iterations']))
        plt.legend(loc='best')
        plt.grid()
